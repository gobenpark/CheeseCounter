...
class SympathyViewController: BaseListViewController, IndicatorInfoProvider{
  ...
  override func request(requestType: RequestAction) {
    switch requestType{
    case .reload:
      provider.request(.getEmpathyList(pageNum: "0"))
        .filter(statusCode: 200)
        .map(MainSurveyList.self)
    ...
    case .paging(let pageNum):
      provider.request(.getEmpathyList(pageNum: pageNum))
        .filter(statusCode: 200)
        .map(MainSurveyList.self)
        .map{[CheeseViewModel(items: $0.result.data)]}
...
extension SympathyViewController: DZNEmptyDataSetSource{
  func title(forEmptyDataSet scrollView: UIScrollView!) -> NSAttributedString! {
    return NSAttributedString(string: "아직 공감한 설문이 없습니다.", attributes: [.font: UIFont.CheeseFontMedium(size: 13)])
  }
}
